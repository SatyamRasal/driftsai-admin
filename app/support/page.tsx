...
export default async function SupportPage({ searchParams }: { searchParams: Promise<{ submitted?: string }> }) {
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      {submitted ? <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950 dark:text-emerald-300">Your support request was submitted.</div> : null}
      <Card className="mt-8">
        <form action={createLead} className="grid gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2"><label className="mb-1 block text-sm">Message</label><Textarea name="message" rows={8} placeholder="Describe the issue or the feedback you want to share." required /></div>
